...
- **Onboarding Wizard (Phase 9):** New members land on `/onboarding` (6 steps: welcome → organization → plan → tokens → first project → workspace) with a progress bar that's resumable across sessions. Per-member wizard progress (`onboarding_step`, `onboarding_state` jsonb, `onboarding_completed_at`) lives on `organization_members` so each member walks through their own first-run flow without overwriting a teammate's. The wizard also collects organization profile fields (`website`, `industry`, `client_type`) which live on `organizations` — those updates are restricted to owner/admin members on the server (`PATCH /api/auth/me/onboarding`), and non-admins still complete the wizard with their submitted values echoed back into per-member state. The `AuthGuard` redirects an active, non-admin member without `member.onboardingCompletedAt` into the wizard, and explicitly skips `/sign-in`, `/sign-up`, `/not-invited`. Public marketing routes (`/`, `/templates`, `/pricing`) render without sign-in via a dedicated `LandingPage`. The full documentation set lives at the repo root: `README.md`, `ROADMAP.md`, `SECURITY_NOTES.md`, `HIPAA_NOTES.md`, `PHASE_2_MIGRATION_PLAN.md`, `AWS_HIPAA_PHASE_3_PLAN.md`, with the `.env.example` annotated by integration group, plus `docs/acceptance-checklist.md` for the MVP sign-off pass and `pnpm --filter @workspace/scripts seed-demo` for a one-shot demo tenant.
